import type { GeneratedFunnelAssets, LandingPageCopy, TestimonialCard, QualificationSection } from "@/types/generation";
...
// ── Qualification section (Why Should / Shouldn't I Apply) ───────────────────

function buildQualificationSection(b: Builder, s: SchemeColors, q: QualificationSection, ctaText: string): void {
  const makeQualCol = (title: string, bullets: string[], bgColor: string, titleBg: string, textColor: string) => {
    const heading = makeHeading(b, title, "h3",
      { color: ss("#000000"), fontSize: sv(22), fontWeight: ss("900"), textAlign: ss("center"),
        lineHeight: ss("1.1"), paddingTop: sv(20), paddingBottom: sv(16),
        backgroundColor: ss(titleBg), letterSpacing: ss("0.04em") },
      { fontSize: sv(18) });
    const items = bullets.map((txt) =>
      makeParagraph(b, txt,
        { color: ss(textColor), fontSize: sv(15), lineHeight: ss("1.65"),
          paddingTop: sv(6), paddingBottom: sv(6), textAlign: ss("center") })
    );
    return makeCol(b, [heading, ...items], 50, { padH: 32, padV: 24 });
  };

  const leftCol  = makeQualCol("WHY SHOULDN'T I APPLY?", q.shouldntApply, "#c0392b", "#e74c3c", "#ffffff");
  const rightCol = makeQualCol("WHY SHOULD I APPLY?",   q.shouldApply,   "#27ae60", "#2ecc71", "#ffffff");

  const cta = makeButton(b, `${ctaText} →`, "next-step", "",
    { backgroundColor: ss(s.primary), boxShadow: ss(`0 8px 24px ${s.primary}44`) });
  const ctaCol = makeCol(b, [cta], 100, { align: "center", padH: 24 });
  const ctaRow = makeRow(b, [ctaCol], 480);

  const r = makeRow(b, [leftCol, rightCol], 1100, 0);
  makeSection(b, [r, ctaRow], { bgColor: "#0f172a", ptD: 72, pbD: 72, ptM: 48, pbM: 48 });
}

// ── Included recap (clean single-column bullet list) ─────────────────────────

function buildIncludedRecapList(b: Builder, s: SchemeColors, lp: LandingPageCopy, concept: string): void {
  const eyebrow = makeParagraph(b, "Need A Recap?",
    { color: ss(s.primary), fontSize: sv(13), fontWeight: ss("700"), letterSpacing: ss("0.10em"),
      textTransform: ss("uppercase"), paddingBottom: sv(8), textAlign: ss("center") });
  const h2 = makeHeading(b, `Everything You Get When You Join The ${concept}`, "h2",
    { color: ss("#111827"), fontSize: sv(36), fontWeight: ss("900"), lineHeight: ss("1.15"),
      textAlign: ss("center"), paddingBottom: sv(32), maxWidth: sv(720), marginLeft: ss("auto"), marginRight: ss("auto") },
    { fontSize: sv(24), paddingBottom: sv(24) });
  const list = makeBulletList(b, lp.bulletPoints.slice(0, 7), s.primary,
    { fontSize: sv(16), paddingBottom: sv(24) });
  const desc = makeParagraph(b, lp.subheadline,
    { color: ss("#374151"), fontSize: sv(15), lineHeight: ss("1.75"),
      paddingTop: sv(8), paddingBottom: sv(28), maxWidth: sv(700), marginLeft: ss("auto"), marginRight: ss("auto") });
  const cta = makeButton(b, `${lp.ctaText} →`, "next-step", "",
    { backgroundColor: ss(s.primary), boxShadow: ss(`0 8px 24px ${s.primary}44`) });

  const col = makeCol(b, [eyebrow, h2, list, desc, cta], 100, { align: "center", padH: 40 });
...
function buildHeroCentered(b: Builder, s: SchemeColors, lp: LandingPageCopy, concept: string): void {
  ...
  if (lp.urgencyIdeas[0]) {
    elIds.push(makeParagraph(b, lp.urgencyIdeas[0],
      { color: ss("#f87171"), fontSize: sv(13), fontWeight: ss("600"), paddingTop: sv(14), textAlign: ss("center") }));
  }
...
function buildSocialProofStarsBullets(b: Builder, s: SchemeColors, corePromise: string): void {
  const stars   = makeParagraph(b, "⭐⭐⭐⭐⭐  500+ coaches launched", { color: ss("#94a3b8"), fontSize: sv(13), textAlign: ss("center") });
  const sep     = makeParagraph(b, "·", { color: ss("rgba(255,255,255,0.2)"), fontSize: sv(20), textAlign: ss("center") });
  const promise = makeParagraph(b, `✓  ${corePromise}`, { color: ss("#94a3b8"), fontSize: sv(13), textAlign: ss("center") });
  const sep2    = makeParagraph(b, "·", { color: ss("rgba(255,255,255,0.2)"), fontSize: sv(20), textAlign: ss("center") });
  const noCard  = makeParagraph(b, "✓  No credit card required", { color: ss("#94a3b8"), fontSize: sv(13), textAlign: ss("center") });
...
function buildSocialProofHorizontalBadges(b: Builder, s: SchemeColors, corePromise: string): void {
  const t1 = makeParagraph(b, "⭐⭐⭐⭐⭐  500+ coaches", { color: ss("#94a3b8"), fontSize: sv(13), textAlign: ss("center"), fontWeight: ss("500") });
  const t2 = makeParagraph(b, `✓  ${corePromise}`,       { color: ss("#94a3b8"), fontSize: sv(13), textAlign: ss("center"), fontWeight: ss("500") });
  const t3 = makeParagraph(b, "🔒  No credit card required", { color: ss("#94a3b8"), fontSize: sv(13), textAlign: ss("center"), fontWeight: ss("500") });
...
function buildIncludedThreeColChecks(b: Builder, s: SchemeColors, lp: LandingPageCopy, concept: string): void {
  ...
  for (let start = 0; start < allBullets.length; start += perRow) {
    ...
    const colIds = rowBullets.map((b_text) => {
      const icon = makeParagraph(b, "✓", { color: ss(s.primary), fontSize: sv(22), fontWeight: ss("900"), paddingBottom: sv(8) });
      const txt  = makeParagraph(b, b_text, { color: ss("#374151"), fontSize: sv(15), lineHeight: ss("1.6") });
...
function buildIncludedTwoColBullets(b: Builder, s: SchemeColors, lp: LandingPageCopy, concept: string): void {
  ...
  const makeListCol = (items: string[]) => {
    const itemEls = items.flatMap((txt) => [
      makeParagraph(b, "✓", { color: ss(s.primary), fontSize: sv(16), fontWeight: ss("900"), paddingBottom: sv(4) }),
      makeParagraph(b, txt, { color: ss("#374151"), fontSize: sv(15), lineHeight: ss("1.6"), paddingBottom: sv(12) }),
    ]);
    return makeCol(b, itemEls, 50, { padH: 32, padV: 16 });
  };
...
function buildIncludedImageLeftList(b: Builder, s: SchemeColors, lp: LandingPageCopy, concept: string): void {
  ...
  const listItems = lp.bulletPoints.slice(0, 6).flatMap((txt) => [
    ...
    makeParagraph(b, txt, { color: ss("#374151"), fontSize: sv(15), lineHeight: ss("1.55"), paddingBottom: sv(10) }),
  ]);
...
function buildIncludedSingleColNumbered(b: Builder, s: SchemeColors, lp: LandingPageCopy, concept: string): void {
  ...
  const itemEls = bullets.flatMap((txt, i) => {
    ...
    const label = makeParagraph(b, txt,
      { color: ss("#374151"), fontSize: sv(16), lineHeight: ss("1.6"), paddingBottom: sv(24) });
...
function buildIncludedAlternatingRows(b: Builder, s: SchemeColors, lp: LandingPageCopy, concept: string): void {
  ...
  const altRows = pairs.map((txt, i) => {
    ...
    const label   = makeParagraph(b, txt,
      { color: ss("#374151"), fontSize: sv(16), lineHeight: ss("1.65") });
...
function buildFaqTwoCol(b: Builder, s: SchemeColors, lp: LandingPageCopy): void {
  ...
  const headingRow = makeRow(b, [headingCol], 800, 0);
  const items = lp.faqItems.slice(0, 6);
  ...
  const makeQaCol = (faqs: typeof items) => {
    ...
    return makeCol(b, els, 50, { padH: 24 });
  };
...
function buildCtaTwoColForm(b: Builder, s: SchemeColors, lp: LandingPageCopy): void {
  const h2 = makeHeading(b, "Ready to start? Spots are limited.", "h2",
    { color: ss("#ffffff"), fontSize: sv(36), fontWeight: ss("900"), lineHeight: ss("1.15"), paddingBottom: sv(12) },
    { fontSize: sv(24) });
  ...
  const cta = makeButton(b, `${lp.ctaText} →`, "next-step", "",
    { backgroundColor: ss("#ffffff"), color: ss(s.primary), boxShadow: ss("0 8px 24px rgba(0,0,0,0.2)") });
...
function buildCtaWithCountdown(b: Builder, s: SchemeColors, lp: LandingPageCopy): void {
  const h2 = makeHeading(b, "Ready to start? Spots are limited.", "h2",
    { color: ss("#ffffff"), fontSize: sv(40), fontWeight: ss("900"), textAlign: ss("center"), lineHeight: ss("1.12"), paddingBottom: sv(8), maxWidth: sv(600), marginLeft: ss("auto"), marginRight: ss("auto") },
    { fontSize: sv(26) });
...
function buildCtaDarkMinimal(b: Builder, s: SchemeColors, lp: LandingPageCopy): void {
  ...
  const h2 = makeHeading(b, "Ready to start? Spots are limited.", "h2",
    { color: ss("#f8fafc"), fontSize: sv(44), fontWeight: ss("900"), textAlign: ss("center"), lineHeight: ss("1.1"), paddingBottom: sv(36), maxWidth: sv(560), marginLeft: ss("auto"), marginRight: ss("auto") },
    { fontSize: sv(26) });
  const cta = makeButton(b, `${lp.ctaText} →`, "next-step", "",
...
function dispatchWhatsIncluded(b: Builder, s: SchemeColors, lp: LandingPageCopy, variant: string, concept: string): void {
  const VALID = new Set(["included-three-col-checks","included-two-col-bullets","included-image-left-list","included-single-col-numbered","included-alternating-rows","included-recap-list"]);
  ...
  switch (v) {
    ...
    case "included-recap-list":          return buildIncludedRecapList(b, s, lp, concept);
...
export function buildLandingPageData(data: GeneratedFunnelAssets): GhlPageData {
  ...
  // Qualification section — who should / shouldn't apply
  if (data.qualificationSection) {
    buildQualificationSection(b, s, data.qualificationSection, lp.ctaText);
  }
...
export function buildRegistrationPageData(data: GeneratedFunnelAssets): GhlPageData {
  const b = createBuilder();
  const s       = getScheme(data.colourScheme);
  const lp      = data.landingPage;
  const concept = data.offerSummary.challengeConcept ?? "30-Day Challenge";

  // Hero with headline + form
  {
    const badge = makeParagraph(b,
      "Limited Spots Available — Register Free",
      { color: ss(s.primary), fontSize: sv(13), fontWeight: ss("700"), textAlign: ss("center"), paddingBottom: sv(16), letterSpacing: ss("0.06em"), textTransform: ss("uppercase") },
    );
    const h1 = makeHeading(b,
      lp.headlineOptions[0] ?? `Join the ${concept}`, "h1",
      { color: ss("#ffffff"), fontSize: sv(48), fontWeight: ss("900"), textAlign: ss("center"), lineHeight: ss("1.1"), paddingBottom: sv(14), maxWidth: sv(640), marginLeft: ss("auto"), marginRight: ss("auto") },
      { fontSize: sv(30) },
    );
    const sub = makeParagraph(b,
      lp.subheadline,
      { color: ss("rgba(255,255,255,0.78)"), fontSize: sv(17), textAlign: ss("center"), lineHeight: ss("1.7"), maxWidth: sv(520), marginLeft: ss("auto"), marginRight: ss("auto"), paddingBottom: sv(32) },
    );
    const formEl = makeForm(b);
    const trust = makeParagraph(b,
      "🔒  Free to join · No credit card required",
      { color: ss("rgba(255,255,255,0.45)"), fontSize: sv(12), textAlign: ss("center"), paddingTop: sv(14) },
    );
    const c = makeCol(b, [badge, h1, sub, formEl, trust], 100, { align: "center", padH: 32 });
    const r = makeRow(b, [c], 560);
    makeSection(b, [r], {
      bg: `linear-gradient(160deg, ${s.dark} 0%, ${s.mid} 60%, ${s.dark} 100%)`,
      ptD: 88, pbD: 88, ptM: 56, pbM: 56,
    });
  ...
  // Bullet points — what they get
  {
    ...
    const h2 = makeHeading(b,
      `Here's what's inside the ${concept}`, "h2",
      { color: ss("#111827"), fontSize: sv(32), fontWeight: ss("900"), textAlign: ss("center"), lineHeight: ss("1.2"), paddingBottom: sv(32) },
      { fontSize: sv(22) },
    );
    const list = makeBulletList(b, lp.bulletPoints.slice(0, 6), s.primary, { fontSize: sv(15), paddingBottom: sv(20) });
    const cta = makeButton(b, `${lp.ctaText} →`, "next-step", "",
      { backgroundColor: ss(s.primary), boxShadow: ss(`0 8px 24px ${s.primary}44`) },
    );
    const c = makeCol(b, [eyebrow, h2, list, cta], 100, { align: "center", padH: 40 });
    const r = makeRow(b, [c], 760);
    makeSection(b, [r], { bgColor: "#ffffff", ptD: 72, pbD: 72, ptM: 48, pbM: 48 });
  }
...
export function buildApplicationFormPageData(data: GeneratedFunnelAssets): GhlPageData {
  ...
  const concept = os.challengeConcept ?? "30-Day Challenge";
  ...
  // Hero
  {
    ...
    const h1 = makeHeading(b,
      `Apply for the ${concept}`, "h1",
      { color: ss("#ffffff"), fontSize: sv(44), fontWeight: ss("900"), textAlign: ss("center"), lineHeight: ss("1.12"), paddingBottom: sv(14), maxWidth: sv(620), marginLeft: ss("auto"), marginRight: ss("auto") },
      { fontSize: sv(28) },
    );
    const sub = makeParagraph(b,
      os.corePromise,
      { color: ss("rgba(255,255,255,0.75)"), fontSize: sv(17), textAlign: ss("center"), lineHeight: ss("1.7"), maxWidth: sv(500), marginLeft: ss("auto"), marginRight: ss("auto") },
    );
    const c = makeCol(b, [badge, h1, sub], 100, { align: "center", padH: 32 });
    const r = makeRow(b, [c], 800);
    makeSection(b, [r], {
      bg: `linear-gradient(160deg, ${s.dark} 0%, ${s.mid} 60%, ${s.dark} 100%)`,
      ptD: 80, pbD: 80, ptM: 52, pbM: 52,
    });
  }
  ...
  {
    const intro = makeParagraph(b,
      `Tell us a little about yourself so we can make sure the ${concept} is the right fit.`,
      { color: ss("#374151"), fontSize: sv(16), textAlign: ss("center"), lineHeight: ss("1.7"), maxWidth: sv(560), marginLeft: ss("auto"), marginRight: ss("auto"), paddingBottom: sv(32) },
    );
    const formEl = makeForm(b);
    const trust = makeParagraph(b,
      "Your answers are confidential and only used to personalise your experience.",
      { color: ss("#9ca3af"), fontSize: sv(12), textAlign: ss("center"), paddingTop: sv(16) },
    );
    const c = makeCol(b, [intro, formEl, trust], 100, { align: "center", padH: 32 });
    const r = makeRow(b, [c], 600);
    makeSection(b, [r], { bgColor: "#f8fafc", ptD: 72, pbD: 80, ptM: 48, pbM: 56 });
  }

  // Who it's for — qualification bullets
  if (data.qualificationSection) {
    const q = data.qualificationSection;
    const eyebrow = makeParagraph(b,
      "Is This Right For You?",
      { color: ss(s.primary), fontSize: sv(11), fontWeight: ss("700"), textAlign: ss("center"), letterSpacing: ss("0.12em"), textTransform: ss("uppercase"), paddingBottom: sv(8) },
    );
    const h2 = makeHeading(b,
      "This programme is designed for:", "h2",
      { color: ss("#ffffff"), fontSize: sv(30), fontWeight: ss("900"), textAlign: ss("center"), lineHeight: ss("1.2"), paddingBottom: sv(28) },
      { fontSize: sv(22) },
    );
    const items = q.shouldApply.slice(0, 5).map((item) =>
      makeParagraph(b, `✓  ${item}`, {
        color: ss("rgba(255,255,255,0.85)"), fontSize: sv(15), lineHeight: ss("1.65"), paddingBottom: sv(8), textAlign: ss("center"),
      })
    );
    const c = makeCol(b, [eyebrow, h2, ...items], 100, { align: "center", padH: 40 });
    const r = makeRow(b, [c], 680);
    makeSection(b, [r], {
      bg: `linear-gradient(160deg, ${s.dark} 0%, ${s.mid} 60%, ${s.dark} 100%)`,
      ptD: 64, pbD: 64, ptM: 40, pbM: 40,
    });
  }

  // Benefit bar
  {
    const benefits = [lp.bulletPoints[0], lp.bulletPoints[1], lp.bulletPoints[2]].filter(Boolean);
    const cols = benefits.map((b_) =>
      makeCol(b, [makeParagraph(b, `✓  ${b_}`, { color: ss("#374151"), fontSize: sv(14), textAlign: ss("center"), lineHeight: ss("1.5") })], 33, { align: "center", padH: 16 })
    );
    if (cols.length > 0) {
      const r = makeRow(b, cols, 960, 16);
      makeSection(b, [r], { bgColor: "#ffffff", ptD: 48, pbD: 48, ptM: 32, pbM: 32 });
    }
  }
...
export function buildAppReceivedPageData(data: GeneratedFunnelAssets): GhlPageData {
  const b = createBuilder();
  const s       = getScheme(data.colourScheme);
  const ty      = data.thankYouPage;
  const concept = data.offerSummary.challengeConcept ?? "30-Day Challenge";
  ...
  // Hero — confirmation
  {
    ...
    const h1 = makeHeading(b,
      ty.confirmationMessage, "h1",
      { color: ss("#ffffff"), fontSize: sv(46), fontWeight: ss("900"), textAlign: ss("center"), lineHeight: ss("1.12"), paddingBottom: sv(20), maxWidth: sv(700), marginLeft: ss("auto"), marginRight: ss("auto") },
      { fontSize: sv(28) },
    );
    const sub = makeParagraph(b,
      `We review every application personally. You'll hear back within 24 hours about your spot in the ${concept}.`,
  ...
  // What happens next
  {
    ...
    const stepEls = ty.nextSteps.flatMap((step, i) => [
      makeParagraph(b, `${i + 1}. ${step}`, {
        ...
        paddingTop: sv(16), paddingBottom: sv(16),
        paddingLeft: sv(20), paddingRight: sv(20),
        backgroundColor: ss("#f8fafc"), borderRadius: sv(12), marginBottom: sv(10),
      }),
    ]);
    const c = makeCol(b, [eyebrow, h2, ...stepEls], 100, { padH: 0 });
    const r = makeRow(b, [c], 620, 24);
  ...
  // Accelerate your results — book a strategy call
  {
    const h2 = makeHeading(b,
      "While you wait — book your strategy call", "h2",
      { color: ss("#ffffff"), fontSize: sv(36), fontWeight: ss("900"), textAlign: ss("center"), lineHeight: ss("1.15"), paddingBottom: sv(14), maxWidth: sv(600), marginLeft: ss("auto"), marginRight: ss("auto") },
      { fontSize: sv(24) },
    );
    const sub = makeParagraph(b,
      ty.bookingEncouragement,
      { color: ss("rgba(255,255,255,0.8)"), fontSize: sv(16), textAlign: ss("center"), lineHeight: ss("1.7"), maxWidth: sv(500), marginLeft: ss("auto"), marginRight: ss("auto"), paddingBottom: sv(36) },
    );
    const cta = makeButton(b,
      "Book My Strategy Call →", "next-step", "",
      { backgroundColor: ss("#ffffff"), color: ss(s.primary), boxShadow: ss("0 8px 24px rgba(0,0,0,0.15)") },
    );
    const c = makeCol(b, [h2, sub, cta], 100, { align: "center", padH: 32 });
    const r = makeRow(b, [c], 640);
...
export function buildStrategyCallPageData(data: GeneratedFunnelAssets): GhlPageData {
  const b = createBuilder();
  const s       = getScheme(data.colourScheme);
  const bk      = data.bookingPage;
  const os      = data.offerSummary;
  ...
  // Two-column: why book + calendar
  {
    ...
    const whoLabel = makeParagraph(b,
      `This call is for you if:`,
      { color: ss(s.accent), fontSize: sv(13), fontWeight: ss("700"), paddingTop: sv(20), paddingBottom: sv(8) },
    );
    const whoText = makeParagraph(b,
      os.targetAudienceSummary,
      { color: ss("#374151"), fontSize: sv(13), lineHeight: ss("1.6") },
    );
    const trustItems = ["Free 30-minute call", "No sales pressure", "100% confidential"].map((t) =>
      makeParagraph(b, `✓  ${t}`, { color: ss("#6b7280"), fontSize: sv(13), paddingBottom: sv(6) })
    ...
    const leftCol = makeCol(b,
      [whyLabel, ...whyItems, expectLabel, expectText, whoLabel, whoText, makeDivider(b), ...trustItems],
      50, { padH: 24 },
    );
...
export interface AllPageData {
  ...
  booking:         GhlPageData;
  registration:    GhlPageData;
  applicationForm: GhlPageData;
  appReceived:     GhlPageData;
  strategyCall:    GhlPageData;
}
...
export function buildAllPageData(data: GeneratedFunnelAssets): AllPageData {
  return {
    ...
    registration:    buildRegistrationPageData(data),
    applicationForm: buildApplicationFormPageData(data),
    appReceived:     buildAppReceivedPageData(data),
    strategyCall:    buildStrategyCallPageData(data),
  };
}
