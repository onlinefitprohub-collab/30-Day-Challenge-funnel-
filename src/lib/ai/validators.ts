...
export const applicationLandingPageSchema = z.object({
  // ── Fields injected into the GHL template ────────────────────────────────
  ...
  textTestimonials:             z.array(textTestimonialSchema).min(1).max(5),
  ...
  // ── Generated but not yet injected — optional so one missing field can't ──
  // ── break the entire call and fall back to mock                          ──
  videoSectionSubheading:       z.string().optional(),
  testimonialIntroHeading:      z.string().optional(),
  testimonialVideoQuote:        z.string().optional(),
  credentialItems:              z.array(credentialItemSchema).optional(),
  benefitBlocks:                z.array(benefitBlockSchema).optional(),
  midCtaHeading:                z.string().optional(),
  midCtaText:                   z.string().optional(),
  galleryHeading:               z.string().optional(),
  shouldNotApply:               z.array(z.string()).optional(),
