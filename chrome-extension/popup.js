...
// Debug panel: hidden by default, toggle by clicking the version footer
...
/* ════════════════════════════════════════════════════════════════════════════
   DEBUG PANEL TOGGLE — click version number to show/hide
   ════════════════════════════════════════════════════════════════════════════ */
...
function initDebugToggle() {
  ...
  toggle.addEventListener("click", () => {
    ...
    toggle.style.color  = visible ? "" : "#f97316";
  });
}
...
async function showInjectDebug() {
  ...
  lines.push("=== FitPro Launch Extension v2.61.0 ===");
  lines.push("REMINDER: If version above is NOT 2.61.0, reload the extension in chrome://extensions then hard-refresh GHL.");
...
async function doValidateAISchema() {
  ...
  try {
    ...
    const STYLES_TEXT   = ["fontSize", "fontWeight", "color", "lineHeight", "textAlign", "letterSpacing", "textTransform", "textShadow"];
    ...
    const EXTRA_TEXTISH = ["text", "typography", "inlineTypographies", "icon", "mobileFontSize", "desktopFontSize"];
...
async function doRoundtripTest() {
  ...
  try {
    ...
    if (d.firstSecHasElements !== undefined) lines.push(`sec0 hasElements: ${d.firstSecHasElements}`);
    ...
    if (d.patchStatus1 !== undefined || d.patchToken !== undefined) {
      lines.push(`patchToken: ${d.patchToken ?? "?"} patchTokenOk=${d.patchTokenOk ?? "?"} (s1=${d.patchStatus1 ?? "?"} s2=${d.patchStatus2 ?? "n/a"})`);
    }
    if (d.patchReturnedToken !== undefined) lines.push(`patchReturnedToken: ${d.patchReturnedToken}`);
    if (d.publicReadAfterPatch !== undefined) lines.push(`publicRead AFTER patch (old token, no auth): ${JSON.stringify(d.publicReadAfterPatch)}`);
...
async function doApiLog() {
  ...
  try {
    ...
    lines.push(`★ = 201 response — save-endpoint candidates.`);
    lines.push(`─────────────────────────────────────────────────────────`);
    if (log.length === 0) {
      ...
      log.forEach((e, idx) => {
        ...
        if (e.is201 || e.status === 201) {
          ...
          if (reqBody) {
            ...
            if (reqBody.length > 1200) lines.push(`    ... (${reqBody.length - 1200} more chars)`);
          } else {
            lines.push(`    REQ: (none)`);
          }
          lines.push(`    ─────────────────────────────────────────────────────`);
...
async function doNativeFirebasePayload() {
  ...
  try {
    ...
    if (!res?.ok) {
      ...
      lines.push("Steps: open a GHL page in the builder, then click this button.");
      div.textContent = lines.join("\n");
      div.className   = "paste-result info";
    } else {
      ...
      if (res.raw && res.raw.length > 5000) {
        lines.push(`… (${res.raw.length - 5000} more chars — use Copy to get all)`);
      }
      div.textContent = lines.join("\n");
...
function doDownloadNativeFirebase() {
  ...
  sendMessage({ type: "CF_GET_FULL_NATIVE_JSON" }).then(function(res) {
    if (btn) { btn.disabled = false; btn.textContent = "⬇ Download Full JSON"; }
    if (!res?.ok) {
