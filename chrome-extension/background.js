      ...
      if (fbMatch) {
        ...
        if (idToken) {
          try {
            ...
            /* Write sections in DUAL FORMAT: each section gets both the flat
             * top-level keys (e.g. section.child, section.styles) from its
             * metaData spread, AND the nested metaData property.
             * Reason: GHL's builder may read section.child (flat) for row IDs
             * rather than section.metaData.child (wrapped).  Dual format works
             * regardless of which path GHL uses.                               */
            const dualSections = (pd.sections ?? []).map(s => {
              const meta = s.metaData ?? s;
              return { id: s.id ?? meta.id, ...meta, metaData: meta };
            });
