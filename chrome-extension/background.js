      ...
      if (fbMatch) {
        ...
        if (idToken) {
          try {
            ...
            /* Use sections in pure wrapped format { id, metaData } — exactly
             * matching GHL's own section format (confirmed firstSec topKeys:
             * ["id","metaData"]).  The dual-format spread we tried in v2.21.0
             * caused TypeError: o1.elements is not iterable in GHL's
             * createNestedJsonFromSections because the extra top-level keys
             * (including `element`) confused GHL's internal page loader.      */
            const writePayload = {
